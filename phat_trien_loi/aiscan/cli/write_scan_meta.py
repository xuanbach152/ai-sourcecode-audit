...
def tree_extent(scan_root: str) -> Extent | None:
    ...
    so the report can say it was not followed. `.git` and `AISCAN-*`
    ...
    if listing:
        ...
        try:
            ...
            return None
        names.discard(".git")
    kept = sorted(n for n in names if not n.startswith(identity.REPORT_DIR_PREFIX))
...
def target_files(scan_root: str, scope: list[str]) -> list[str] | None:
    ...
    return sorted({
        ...
        for path in listing.split("\0")
        if path
        and not path.startswith(identity.REPORT_DIR_PREFIX)
...
def worktree_dirty(scan_root: str) -> bool | None:
    ...
    for line in status.splitlines():
        if len(line) < len("XY P"):
            continue
        path = line[3:].split(" -> ")[-1]
        if any(part.startswith(identity.REPORT_DIR_PREFIX) for part in path.split("/")[:-1]):
            continue
        return True
    return False
...
def parse_options(argv: list[str]) -> Args:
    ...
    ap.add_argument("--mode", required=True, choices=identity.MODES)
...
def main(argv: list[str]) -> int:
    ...
    if files is not None:
        (run_dir / identity.TARGET_FILES_NAME).write_bytes((strictjson.text(files) + "\n").encode())
