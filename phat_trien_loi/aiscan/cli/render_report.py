...
Writes AISCAN-RESULTS.jsonl (one finding per line, fields in a fixed
order), AISCAN-RESULTS.sarif (the same findings as a SARIF 2.1.0 log)
and the AISCAN-REVISION-<tag>.json stamp, places the report markdown
...
from aiscan.lib import absolute, console, cwe, identity, sarif, secret, source, strictjson
from aiscan.lib.chain import chain_of, pending_ranks
...
REVISION_PREFIX = "AISCAN-REVISION-"
STAMP_ONLY = frozenset({"duration_s", "verification_runs", "reason_kind"})
JSONL_NAME = "AISCAN-RESULTS.jsonl"
SARIF_NAME = "AISCAN-RESULTS.sarif"
...
def read_votes(run_dir: Path) -> JsonMap | None:
    ...
    if raw.get("provenance") != identity.VOTES_PROVENANCE:
        return None
    return raw
...
def revision_tag(revision: object) -> str:
    ...
    if not sha:
        return "UNVERSIONED"
    if not is_str(sha) or not identity.SHA_RE.match(sha):
...
def scan_of(meta: JsonMap) -> Meta:
    ...
    if value is None or value.version is None or not 1 <= value.version <= 5:
        ...
        raise RenderError(msg)
    mode = meta.get("mode")
    if not is_str(mode) or mode not in identity.MODES:
    ...
    if is_map(revision) and revision.get("dirty") is False:
        ...
        clean_commit = commit if is_str(commit) and identity.SHA_RE.match(commit) else None
    scan = sarif.Scan(
        id=value,
        mode=mode,
        prefix=prefix,
...
def render(run_dir: Path, products_dir: Path) -> Rendered:
    ...
    markdown_path = run_dir / "AISCAN-RESULTS.md"
    if not os.path.isfile(markdown_path):
        raise RenderError(
            "AISCAN-RESULTS.md is missing. Write the human-readable "
    ...
    try:
        ...
        msg = f"AISCAN-RESULTS.md is not valid UTF-8: {error}"
        raise RenderError(msg) from error
    ...
    sarif_log = sarif.log(findings, scan, identity.version(), run_properties, panels, notifications)
    ...
    markdown_out = products_dir / "AISCAN-RESULTS.md"
...
def remove_run_dir(run_dir: Path, products_dir: Path) -> str:
    ...
    if target.name != identity.RUN_DIR_NAME:
        return f"kept {run_dir} (not a {identity.RUN_DIR_NAME} run directory)"
