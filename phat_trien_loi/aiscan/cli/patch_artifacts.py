...
from aiscan.lib import absolute, console, identity, strictjson
from aiscan.lib.strictjson import JsonMap, is_list, is_map, is_str
...
REPORT_DIR_RE = re.compile(rf"^{re.escape(identity.REPORT_DIR_PREFIX)}[0-9][0-9-]*\Z")
...
def header_comment(unit: Unit, base: str, report_ref: str) -> str:
    ...
    lines = [
        f"# AIScan -- suggested patch for {unit['id']}: {unit['title']}",
...
def note_written(unit: Unit, stats: list[DiffStat] | None, check: str, report_ref: str) -> str:
    ...
    lines += [
        ...
        "```",
        "",
        "Or ask AIScan to apply it, or to open a pull request for it.",
        "",
    ]
    return "\n".join(lines)
...
def index_markdown(units: list[Unit], base: str, report_dir_name: str, report_ref: str) -> str:
    ...
    lines += [
        ...
        (
            ...
            "applying a patch and opening a merge request stays a separate, "
            "explicit step you drive."
        ),
        "",
    ]
    return "\n".join(lines)
...
def resolve_report_dir(patches_dir: Path) -> Path:
    ...
    if not REPORT_DIR_RE.match(report_dir.name):
        msg = (
            "patches dir must live inside a AISCAN-<timestamp> report "
...
def refuse_reason(path: Path) -> str | None:
    ...
    Only `<report>/.aiscan-run/patch-<ts>/scratch-F<n>` holding its
    ...
    if leaf.parents[1].name != identity.RUN_DIR_NAME:
        return f"its run directory is not inside {identity.RUN_DIR_NAME}/"
...
def remove_patch_run(patch_dir: Path) -> tuple[list[Path], list[str]]:
    ...
    `<report>/.aiscan-run/patch-<ts>` layout is deleted.
    """
    removed: list[Path] = []
    ...
    if run_dir.name != identity.RUN_DIR_NAME:
        return removed, [f"left '{patch_dir}' in place: it is not inside {identity.RUN_DIR_NAME}/"]
    try:
...
def main(argv: list[str]) -> int:
    ...
    if not identity.SHA_RE.match(args.base):
