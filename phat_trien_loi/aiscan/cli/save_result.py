#!/usr/bin/env python3
"""Ghi ket qua mot luot quet vao thu muc chay, va in ra buoc tiep theo.

Doc file JSON ket qua ma bo dieu phoi ghi ra sau moi luot, gop findings,
votes va coverage vao findings.json, votes.json, coverage.json trong thu muc
chay (noi tiep ket qua cua luot truoc neu co), ghi file ung vien cho luot xac
minh sau, roi in dung mot dong `next:`: tham so cho luot xac minh tiep theo,
hoac lenh viet bao cao. Moi finding duoc ghi tai dong that su chua doan ma no
trich dan, kem theo so dong ma nguoi tim ra khai bao.

Usage:
  save_result.py <output_file> <run_dir>

Thoat 0 khi ket qua da duoc ghi (hoac da ghi tu truoc), 1 khi khong ghi duoc
(dong `next:` van noi ro phai lam gi), 2 khi sai tham so dong lenh.
Tuong thich Python 3.9, chi dung thu vien chuan.
"""
...
from aiscan.lib import console, identity, source, strictjson
from aiscan.lib.chain import Chain, chain_of, pending_ranks
from aiscan.lib.finding import (
    CONFIDENCE_RANK,
    SEVERITIES,
    FindingError,
    file_field,
...
from aiscan.lib.strictjson import JsonMap, is_int, is_list, is_map, is_str

# So dong moi file ung vien; bo dieu phoi phai suy ra rank tung file tu dung con so nay.
CHUNK = 25
UNACCOUNTED_ECHO_CAP = 40
CID_RE = re.compile(r"^C([1-9][0-9]*)\Z")
WRITE_REPORT = (
    "next: write AISCAN-RESULTS.md from findings.json and coverage.json per the "
...
def scan_settings(run_dir: Path) -> JsonMap:
    ...
    if run_dir.name != identity.RUN_DIR_NAME:
        msg = f"{run_dir} is not a {identity.RUN_DIR_NAME} run directory"
        raise CannotContinueError(msg)
    try:
...
def records_in(result: JsonMap) -> tuple[Records, dict[int, JsonMap]]:
    ...
    if not is_list(raw) or len(findings) != len(raw):
        ...
        raise NotAResultError(msg)
    if not is_map(votes) or votes.get("provenance") != identity.VOTES_PROVENANCE:
...
def checked(coverage: JsonMap, meta: JsonMap, run_dir: Path) -> JsonMap:
    ...
    if not is_map(research) or research.get("checkable") is not True:
        return coverage
    listing = run_dir / identity.TARGET_FILES_NAME
    try:
...
def next_step(run_dir: Path, meta: JsonMap, chain: Chain) -> str:
    """Dong `next:`: tham so cho luot panel phan con lai, hoac lenh viet bao cao."""
    ...
    return (
        "next: chay them mot luot xac minh voi dung tham so duoi day, roi chay lai "
        "save_result.py tren file ket qua cua luot do" + chr(10)
        + f"verify-args: {strictjson.text(args)}"
    )
